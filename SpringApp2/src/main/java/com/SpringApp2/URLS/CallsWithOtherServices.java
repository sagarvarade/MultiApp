...
public class CallsWithOtherServices {
	...
	private final String SPRING_APP_ONE = env.getProperty("multiapp.springapp1");

	public HttpResponse<String> checkToken(String token) {
		try {
			logger.info("Checking token , {} ", token);
			HttpRequest request = HttpRequest.newBuilder().uri(new URI(SPRING_APP_ONE + "/auth/testtoken"))
